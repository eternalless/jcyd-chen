...
<template>
  <div style="height: 100%;overflow: hidden;">
    <el-row>
      <el-col :span="24">
        <el-row><el-col :span="24"><div class="grid-content right-border"  ><label>当前航班 Flight Infos</label></div></el-col></el-row>
...
<script>
  ...
  export default {
    ...
    computed: {
      showBeltFlightList(){
        const self = this
        this.beltFlightLeftListAll = this.fillBeltArray(this.beltFlightListAll.filter(item => { return item.beltNo === self.beltNo }),3)
    ...
    methods: {
      ...
      selectBeltFlights (val) {
        const param = {beltNo : val}
        listBeltFlights(param).then((res) => {
